...
def sampling(ratio_list, num_list, gt_reshape, class_count, Flag):#0605 version
    all_label_index_dict, train_label_index_dict, val_label_index_dict, test_label_index_dict = {}, {}, {}, {}
    all_label_index_list, train_label_index_list, val_label_index_list, test_label_index_list = [], [], [], []
    ...
    for cls in range(class_count):
        ...
        # Split by num per class
            ...
            total_cls = len(cls_index)
            # 核心规则：>40 用 30/10，否则 10/5
            if total_cls > (num_list[0]+num_list[1]):
                train_index_flag = num_list[0]
                val_index_flag = num_list[1]
            else:
                train_index_flag = num_list[0] // 3
                val_index_flag = num_list[1] // 2

            # 最小值保护（和flag=0对齐）
            train_index_flag = max(train_index_flag, 3)
            val_index_flag = max(val_index_flag, 1)

            # 安全限制：防止val越界
            remaining = total_cls - train_index_flag
            val_index_flag = min(val_index_flag, remaining)
...
#
#         np.random.shuffle(cls_index)
#         total_samples = len(cls_index)
#
#         if Flag == 0:  # 按固定比例划分
#             train_index_flag = max(int(ratio_list[0] * total_samples), 3)  # 每类至少3个
#             val_index_flag = max(int(ratio_list[1] * total_samples), 1)  # 每类至少1个
#
#             # 安全保障：如果训练+验证的数量超过了总数，强行留出至少1个样本给测试集
#             if train_index_flag + val_index_flag >= total_samples:
#                 train_index_flag = max(1, total_samples - 2)
#                 val_index_flag = 1
#
#         elif Flag == 1:  # 按固定数量划分
#             if total_samples > num_list[0]:
#                 train_index_flag = num_list[0]
#             else:
#                 # 当总样本数极少（如20个）时，不能硬性分配15个给训练，否则验证和测试就没了
#                 # 这里改为动态分配：总数的 50% 给训练
#                 train_index_flag = max(1, total_samples // 2)
#
#             # 同样防止验证集把剩下的样本全抽光
#             remaining = total_samples - train_index_flag
#             if remaining > num_list[1]:
#                 val_index_flag = num_list[1]
#             else:
#                 # 至少留1个样本给测试集
#                 val_index_flag = max(1, remaining - 1)
#
#         # ==================== 核心修正：使用绝对索引，互不重叠 ====================
#         # 训练集：从 0 到 train_index_flag
#         train_label_index_dict[cls] = list(cls_index[:train_index_flag])
#
#         # 验证集：从 train_index_flag 到 train_index_flag + val_index_flag
#         val_end = train_index_flag + val_index_flag
#         val_label_index_dict[cls] = list(cls_index[train_index_flag:val_end])
#
#         # 测试集：剩下的全部作为测试集（从 val_end 一直到最后）
#         test_label_index_dict[cls] = list(cls_index[val_end:])
#         # =========================================================================
#
#         train_label_index_list += train_label_index_dict[cls]
#         val_label_index_list += val_label_index_dict[cls]
